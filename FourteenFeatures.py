...
features = ['Hematocrit', 'Hemoglobin', 'Platelets', 'MPV', 'Erythrocytes', 'Lymphocytes', 'MCHC', 'Leukocytes', 'Basophils', 'MCH', 'Eosinophils', 'MCV', 'Monocytes', 'RDW']

import pandas as pd
X_test = pd.read_csv('Dataset-1b.csv')
y_test = X_test['CoV-2'].values.astype(np.int8)
X_test = X_test[features].copy()

X_train = pd.read_csv('Dataset-3b.csv')
y_train = X_train['CoV-2'].values.astype(np.int8)
X_train = X_train[features].copy()
spw = (sum(y_train==0))/(sum(y_train==1))
eval_set = [(X_train, y_train), (X_test, y_test)]

variabilities = [[0.018,	0.024,	0.009],
                 [0.0058,	0.0185,	0.00515],
                 [0.01015,	0.0325,	0.0722],
                 [0.01135,	0.0107,	0.0232],
                 [0.009,	0.018,	0.0104],
                 [0.025,	0.083,	0.11],
                 [0.0076,	0.006,	0.0085],
                 [0.019,	0.111,	0.033],
...
res_prtrb = pd.DataFrame(columns=['AUC Score', 'F1 Score', 'Accuracy'])
res_prtrb_base = pd.DataFrame(columns=['AUC Score', 'F1 Score', 'Accuracy'])
...
for t in range(times):
  ...
  res_prtrb.loc[len(res_prtrb.index)]=[AUC, ACC, F1]
  AUC, ACC, F1 = clf.pred_compute(X_test_fuzzy_base, y_test)
  res_prtrb_base.loc[len(res_prtrb_base.index)]=[AUC, ACC, F1]

Metric_Means = []
Metric_StDevs = []
for col in (res_prtrb.columns):
  Metric_Means.append(res_prtrb[col].mean())
  Metric_StDevs.append(res_prtrb[col].std())

Res = pd.DataFrame(index=(res_prtrb.columns.values))
Res['Perturbated [High] Mean'] = Metric_Means
Res['Perturbated [High] StDev'] = Metric_StDevs

Metric_Means = []
Metric_StDevs = []
for col in (res_prtrb_base.columns):
  Metric_Means.append(res_prtrb_base[col].mean())
  Metric_StDevs.append(res_prtrb_base[col].std())

Res['Perturbated [Low] Mean'] = Metric_Means
Res['Perturbated [Low] StDev'] = Metric_StDevs
Res

Res.to_csv("Fourteen-Prtrb1.csv")
...
Res.to_csv("Fourteen-Prtrb2.csv")

#Augmented

res_aug = pd.DataFrame(columns=['AUC Score', 'F1 Score', 'Accuracy'])
res_aug_base = pd.DataFrame(columns=['AUC Score', 'F1 Score', 'Accuracy'])
...
for t in range(times):
  ...
  clf.fit(X_train_fuzzy.values, y_train)
  AUC, ACC, F1 = clf.pred_compute(X_test_fuzzy, y_test)
  res_aug.loc[len(res_aug.index)]=[AUC, ACC, F1]
  AUC, ACC, F1 = clf.pred_compute(X_test_fuzzy_base, y_test)
  res_aug_base.loc[len(res_aug_base.index)]=[AUC, ACC, F1]

res_aug
res_aug.to_csv("14allI-Augm.csv")

res_aug_base
res_aug_base.to_csv("14allI-Augm_base.csv")

Augmented_Mean = []
Augmented_StDev = []
for col in (res_aug.columns):
  Augmented_Mean.append(res_aug[col].mean())
  Augmented_StDev.append(res_aug[col].std())

Res = pd.DataFrame(index=['AUC Score', 'F1 Score', 'Accuracy'])
Res['Augmented Mean'] = Augmented_Mean
Res['Augmented StDev'] = Augmented_StDev

Augmented_Mean = []
Augmented_StDev = []
for col in (res_aug_base.columns):
  Augmented_Mean.append(res_aug_base[col].mean())
  Augmented_StDev.append(res_aug_base[col].std())

Res['Augmented Mean (Baseline)'] = Augmented_Mean
Res['Augmented StDev (Baseline)'] = Augmented_StDev
Res

Res.to_csv("Fourteen-Augmt.csv")
